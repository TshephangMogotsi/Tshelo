...
export async function getApiFundWorkspace(client: SupabaseClient, actorUserId: string, fundId: string): Promise<ApiDataResult<FundWorkspace | null>> {
  ...
  const [contributions, expenses, members, pledges, contributors, sponsorships, permissions] = await Promise.all([
    ...
    client.from('expenses').select('id, vendor_name, description, receipt_url, category, custom_category, amount, created_at, has_open_query, is_sponsored, sponsored_by_user_id, sponsored_by_name').eq('fund_id', fundId).is('deleted_at', null).order('created_at', { ascending: false }),
...
export async function getApiHomeSummary(client: SupabaseClient, actorUserId: string): Promise<ApiDataResult<HomeSummary>> {
  ...
  const [funds, events, links] = await Promise.all([
    ...
    client.from('events').select('id, creator_id, name, status, event_type, event_emoji, event_date, event_time, venue_name, linked_fund_id, created_at').or(eventFilter).is('deleted_at', null).order('created_at', { ascending: false }),
