...
import { getOperationsOverview } from '@/lib/data/operations'
...
export default async function DashboardPage() {
  ...
  const [admin, overview] = await Promise.all([
    adminPromise,
    getOperationsOverview(supabase),
  ])

  const stats = [
    { label: 'Platform users', value: overview.counts.users, hint: 'Registered accounts', icon: Users, tone: 'purple' },
    { label: 'Active funds', value: overview.counts.active_funds, hint: 'Currently operating', icon: HandCoins, tone: 'green' },
    { label: 'Open tickets', value: overview.counts.open_tickets, hint: 'Awaiting support', icon: TicketCheck, tone: 'amber' },
    { label: 'Open disputes', value: overview.counts.open_disputes, hint: 'Need attention', icon: CircleAlert, tone: 'red' },
  ]
      ...
      <section className="dashboard-grid">
        <article className="panel">
          ...
          <div className="compact-list">
            ...
            {!overview.recent_funds.length && <EmptyRow label="No funds are available yet." />}
          </div>
        </article>
        ...
        <article className="panel">
          ...
          <div className="compact-list">
            ...
            {!overview.recent_tickets.length && <EmptyRow label="The support queue is clear." />}
          </div>
        </article>
      </section>
    </AppShell>
