...
import { validateJoinFundRequest } from '@/lib/api/validation'
import { joinApiFund } from '@/lib/data/api'
...
export async function POST(request: Request) {
  ...
  const body = await readValidatedJson(request, requestId, validateJoinFundRequest)
  if (!body.ok) return body.response
  const result = await joinApiFund(authentication.auth.supabase, body.value.code)
