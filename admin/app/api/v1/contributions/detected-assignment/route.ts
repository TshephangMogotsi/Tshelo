...
import { validateDetectedPaymentAssignmentRequest } from '@/lib/api/validation'
import { assignApiDetectedPayment } from '@/lib/data/api'
...
export async function POST(request: Request) {
  ...
  const body = await readValidatedJson(request, requestId, validateDetectedPaymentAssignmentRequest)
  if (!body.ok) return body.response
  const result = await assignApiDetectedPayment(authentication.auth.supabase, body.value)
