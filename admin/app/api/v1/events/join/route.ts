...
import { validateJoinEventRequest } from '@/lib/api/validation'
import { joinApiEvent } from '@/lib/data/api'
...
export async function POST(request: Request) {
  ...
  const body = await readValidatedJson(request, requestId, validateJoinEventRequest)
  if (!body.ok) return body.response
  const result = await joinApiEvent(authentication.auth.supabase, body.value.code)
