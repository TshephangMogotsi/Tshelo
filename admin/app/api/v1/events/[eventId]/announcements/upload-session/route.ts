...
import { validateCreateEventAnnouncementUploadSessionRequest } from '@/lib/api/validation'
import { createApiEventAnnouncementUploadSession } from '@/lib/data/api'
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  const body = await readValidatedJson(request, requestId, validateCreateEventAnnouncementUploadSessionRequest)
  if (!body.ok) return body.response
  const { eventId } = await params
  const result = await createApiEventAnnouncementUploadSession(
