...
import { validateCreateEventFundRequest } from '@/lib/api/validation'
import { createApiEventFund } from '@/lib/data/api'
...
export async function POST(request: Request) {
  ...
  const body = await readValidatedJson(request, requestId, validateCreateEventFundRequest)
  if (!body.ok) return body.response
  const result = await createApiEventFund(authentication.auth.supabase, body.value)
