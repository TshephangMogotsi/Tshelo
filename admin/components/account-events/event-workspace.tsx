...
function Announcements({ workspace, reload }: { workspace: EventWorkspace; reload: () => void }) {
  ...
  return <section className="member-card" id="announcements"><header><div className="member-section-title"><span><BellRing size={18} /></span><h2>Announcements</h2></div></header><div className="member-card-body">{canPost && <form className="member-announcement-form" onSubmit={post}><label><span>Announcement title</span><input name="title" required minLength={3} maxLength={120} placeholder="Announcement title" /></label><label><span>Message</span><textarea name="body" required minLength={3} maxLength={4000} rows={3} placeholder="Write an update for everyone attending…" /></label><button type="submit" disabled={busy}>{busy ? 'Posting…' : 'Post announcement'}</button></form>}<div className="member-announcements">{workspace.announcements.map(item => <article key={item.id}><div><strong>{item.title}</strong><span>{item.author_name} · {formatDate(item.created_at)}</span></div><p>{item.body}</p></article>)}{!workspace.announcements.length && <div className="member-empty">No announcements have been posted.</div>}</div>{error && <p className="member-form-error" role="alert">{error}</p>}</div></section>
}
