...
| Build a fund report snapshot | `get_fund_report_bundle(p_fund_id)` | [`20260818120000_fund_report_snapshot_and_exports.sql`](../../supabase/migrations/20260818120000_fund_report_snapshot_and_exports.sql) | Caller-scoped RLS visibility and one-statement consistency across financial records, membership, sponsorship, awards, audit history, edit history, and prior exports. |
| Account for a report export | `log_fund_export(p_fund_id, p_export_type)` | [`20260818120000_fund_report_snapshot_and_exports.sql`](../../supabase/migrations/20260818120000_fund_report_snapshot_and_exports.sql) | Closed export types, actor derivation from `auth.uid()`, `export_reports` permission, and insertion of server-owned accounting fields. |
...
| Discover an event from a code | `find_event_by_code(p_code)` | [`20260812150000_event_code_join.sql`](../../supabase/migrations/20260812150000_event_code_join.sql), extended by [`20260909130000_event_invite_plus_one_preview.sql`](../../supabase/migrations/20260909130000_event_invite_plus_one_preview.sql) | Code normalization, limited preview fields, linked-fund indicator, already-participating calculation, and a caller-scoped plus-one allowance matched by user ID or verified phone. |
...
| Select or clear an event banner and focal point | `set_event_banner_focal_point(p_event_id, p_file_id, p_focal_x, p_focal_y)` | [`20260909140000_event_banner_focal_points.sql`](../../supabase/migrations/20260909140000_event_banner_focal_points.sql), delegating selection rules to [`20260908120000_event_banners.sql`](../../supabase/migrations/20260908120000_event_banners.sql) | Reuses the locked active-event, participant, file-manager, same-event and image-only checks; atomically selects at most one banner and saves bounded normalized coordinates without changing image bytes. The older `set_event_banner` remains for rolling-client compatibility. |
| Pin or unpin an Overview announcement | `set_event_announcement_pin(p_event_id, p_announcement_id, p_is_pinned)` | [`20260909120000_event_announcement_pins.sql`](../../supabase/migrations/20260909120000_event_announcement_pins.sql) | Active-event and update-manager checks, announcement/event association, row locking and atomic single-pin replacement. |
